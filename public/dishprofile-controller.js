...
app2.factory('getDishData', ['$http', function ($http) {
    //Create request for User data then send it to other Controller
    //More info http://stackoverflow.com/questions/33843861/why-is-this-factory-returning-a-state-object-instead-of-response-data
    //localStorage.loginChefAtHomeEmail = "chanawatnpound@gmail.com"
    var this_name = "Tuna salad";
    ...
    return $http.post(url + "/list/dish", {name: this_name})
        .then(function (response) {
            //console.log(response.data);
            return response.data;
        });
...
app2.controller('MainCtrl', ['$scope', 'getUserData','getDishData', '$http', function ($scope, getUserData, getDishData, $http, $rootScope, $timeout, $modal) {
    ...
    getDishData.then(function (dish) {
        $scope.dish.name = dish.name
        $scope.dish.price = dish.price
        $scope.dish.recipe = dish.recipe
        $scope.dish.ingredients = dish.ingredients
        $scope.dish.inglength = dish.ingredients.length
